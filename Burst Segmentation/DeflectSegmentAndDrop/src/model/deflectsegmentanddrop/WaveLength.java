	...
	public void lifeCycle() {
		while (true) {
			...
			//	model.processing.setText("LastProcessed:  "+nextBurst.getName());
				...
				{Burst nextBurst2 = (Burst) model.getdefQueue2().first();
				nextBurst2.issegmented=true;
				nextBurst2.completed=false;
				model.getdefQueue2().remove(nextBurst2);
				//nextBurst2.activateAfter(nextBurst);
				//nextBurst2.activate(new TimeSpan(0));
					
				
					nextBurst.stop=(((model.getInActionQueue(stock).first().presentTime().getTimeRounded())/100)+3);
					nextBurst.actual=((nextBurst.stop-nextBurst.start)*1);
					model.bgElement11.setText(nextBurst.getName()+"     Preempted for Segmentation   ");
					...
					//model.getBurstQueue().insert(nextBurst2);
...
//					model.bgElement8.setText("LastProcessed:  "+nextBurst.getName());
					...
					if(this.isInterrupted())
					{
						Burst nextBurst2 = (Burst) model.getdefQueue().first();
						nextBurst2.issegmented=true;
						nextBurst2.completed=false;
						model.getdefQueue().remove(nextBurst2);
						//nextBurst2.activateAfter(nextBurst);
						//nextBurst2.activate(new TimeSpan(0));
...
//							model.bgElement8.setText("LastProcessed:  "+nextBurst.getName());
							...
							if(this.isInterrupted())
							{
								Burst nextBurst2 = (Burst) model.getdefQueue3().first();
								nextBurst2.issegmented=true;
								nextBurst2.completed=false;
								model.getdefQueue3().remove(nextBurst2);
								//nextBurst2.activateAfter(nextBurst);
								//nextBurst2.activate(new TimeSpan(0));
